...
class qt_freq_time_sink_iq(gr.hier_block2, Qt.QWidget):
    ...
    def __init__(self,
        ...
        if self.enableTimeWaveform:
            ...
            self.tabs.addTab(self.tabs_widget_2, "%s Time Waveform"%(label,))
        ...
        if self.enableWaterfall:
            ...
            self.tabs_grid_layout_1.addWidget(self._waterfallSink_win, 0,0,1,1)
            ...
            for i in range(2*1):
                if len(labels[i]) == 0:
                    if(i % 2 == 0):
                        ...
                        self.timeSink.set_line_label(i, "Im{{Data {0}}}".format(i/2))
                else:
        ...
        if self.enableSpectrum or self.enableWaterfall:
            self.fftGainMultiplier = blocks.multiply_const_vcc((fftGainLinear, ))
