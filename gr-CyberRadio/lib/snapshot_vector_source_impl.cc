...
snapshot_vector_source::sptr snapshot_vector_source::make(const std::string radio_type,
                                                          const std::string& ip,
                                                          unsigned int port,
                                                          unsigned int block_size,
                                                          unsigned int block_rate)
{
    return gnuradio::make_block_sptr<snapshot_vector_source_impl>(
...
/*******************************************************************************
 * \brief Constructor.
 * \param radio_type The NDR Radio Type
 * \param ip The NDR IP Address
 * \param port The NDR Control Port
 * \param block_size The Block size for vector output.
 * \param block_rate The Rate at which we should output.
 *******************************************************************************/
snapshot_vector_source_impl::snapshot_vector_source_impl(const std::string radio_type,
                                                         const std::string& ip,
                                                         unsigned int port,
                                                         unsigned int block_size,
                                                         unsigned int block_rate)
    : gr::sync_block("snapshot_vector_source",
                     gr::io_signature::make(0, 0, 0),
                     gr::io_signature::make(1, 1, sizeof(output_type) * block_size)),
...
{
    // Main thing we need to do to find out what our iovec should look like.
    this->determine_radio_type();
    ...
    for (int i = 0; i < 3; i++) {
        ...
        msg << "rxVec[" << i << "].iov_len = " << rxVec[i].iov_len;
        GR_LOG_INFO(this->d_logger, msg.str());
    ...
    this->packets_per_block =
        (int)ceil(static_cast<float>(this->block_size) / this->d_samples_per_frame);
...
bool snapshot_vector_source_impl::start(void)
{
    ...
    this->sock_fd =
        this->initSocket(this->d_ip.c_str(), this->d_port); // create recv socket
    if (this->sock_fd < 0) {
        perror("Could not create socket.  Check that /proc/sys/net/core/rmem_max is set "
               "to 268435456");
 ...
 * \brief
 *******************************************************************************/
int snapshot_vector_source_impl::initSocket(const std::string ip, unsigned short port)
{
    // /proc/sys/net/core/rmem_max holds this value must be >= recv_buffer_size
    // linux kernel receiver buffer size. this is not to be confused with the size of this
    // class' ring buffer.
    ...
    if (check_size != 2 * recv_buffer_size) {
        fprintf(stderr,
                "ERROR: set recv buf size, but it is incorrect in kernel. check max "
                "limit in /proc/sys/net/core/rmem_max\n");
        return -1;
    }
...
void snapshot_vector_source_impl::pause()
{
    ...
    // Open a new socket
    this->sock_fd = this->initSocket(this->d_ip.c_str(), this->d_port);
    if (this->sock_fd < 0) {
        perror("Could not create socket.  Check that /proc/sys/net/core/rmem_max is set "
               "to 268435456");
        throw "Socket Creation Error";
                                      ...
                                      gr_vector_void_star& output_items)
{
    ...
    while (this->stream_counter < this->packets_per_block) {
        rxSize = readv(this->sock_fd, this->rxVec, 3);
        if (rxSize > 1000) {
            // std::cout << " . " << std::endl;
            ...
            samps2use = std::min((const int)this->d_samples_per_frame,
                                 (const int)(this->d_block_size - this->sample_counter));
            volk_16i_s32f_convert_32f((float*)((uint8_t*)this->sampleVector.data() +
                                               this->sample_counter * sizeof(gr_complex)),
                                      (int16_t*)(this->rxVec[1].iov_base),
                                      32768.0,
                                      2 * samps2use);
        ...
        if (this->stream_counter >= this->packets_per_block) {
            ...
            memcpy(out, this->sampleVector.data(), this->block_size * sizeof(gr_complex));
...
void snapshot_vector_source_impl::txStatusMsg(void)
{
    ...
    pmt::pmt_t msg = pmt::cons(pmt::intern("Over Range Scenario Detected"), pmt::PMT_NIL);
 ...
 * \return
 *******************************************************************************/
void snapshot_vector_source_impl::determine_radio_type(void)
{
    if ((d_radio_type.compare("ndr308") == 0) ||
        (d_radio_type.compare("ndr308-ts") == 0) ||
        (d_radio_type.compare("ndr318-ts") == 0) ||
        (d_radio_type.compare("ndr318") == 0) || (d_radio_type.compare("ndr318a") == 0) ||
        ...
        (d_radio_type.compare("ndr814") == 0) || (d_radio_type.compare("ndr818") == 0) ||
        (d_radio_type.compare("ndr651") == 0) || (d_radio_type.compare("ndr804") == 0) ||
    ...
    } else if ((d_radio_type.compare("ndr354") == 0) ||
               (d_radio_type.compare("ndr364") == 0) ||
               (d_radio_type.compare("ndr374") == 0)) {
        this->set_iqSwap(false);
        this->set_byteSwap(true);
