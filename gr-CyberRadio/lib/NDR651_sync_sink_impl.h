 ...
 * It takes in 2 complex IQ streams and sends data out via syncTXClient (some
 * sort of ethernet thing) It sends data out in sizes of ETH_PACKET_SIZE (1024),
...
private:
  ...
  double centerFreqMHz[NUM_CH];
  int txChannel[NUM_CH];
  double rf_attenuationDB[NUM_CH];
  const char *eth_index[NUM_CH];
  long upd_port[NUM_CH];
  double frequencyOffsetMHz[NUM_CH];
  double digital_attenuationDB[NUM_CH];
  float mult[NUM_CH];
  int ducChannel[NUM_CH];

  std::vector<LibCyberRadio::NDR651::TXClient *> txClients;
  LibCyberRadio::NDR651::SyncTXClient *syncTXClient;
  // These buffers contain the gr_complex data that's been converted into
  // int16_t IQ data.
...
public:
  ...
  bool setDUCFreq_callback(double frequencyOffsetMHz1,
                           double frequencyOffsetMHz2);
  bool setDUCAtten_callback(double digital_attenuationDB1,
  ...
  bool setDUCParameters(int ducChannel1, int ducChannel2, int channelRate,
                        int txChannel1, int txChannel2);
  bool setEthernetInterface(unsigned int tenGbeIndex, const char *eth_index1,
                            const char *eth_index2, long upd_port1,
                            long upd_port2);
};
