...
vita_multifile_iq_source::sptr vita_multifile_iq_source::make(
    const std::vector<std::string>& filespecs,
    bool alphabetical,
    int vita_type,
    size_t payload_size,
	...
	return gnuradio::get_initial_sptr
	       (new vita_multifile_iq_source_impl(filespecs,
	               alphabetical,
	               vita_type,
	               payload_size,
	               vita_header_size,
...
vita_multifile_iq_source_impl::vita_multifile_iq_source_impl(
	...
	: gr::sync_block(
	  ...
	  d_buffer(NULL),
	  d_buffer_offset(0),
	  d_packet_data_available(false),
	  d_absolute_packet_num(0),
	  d_realtime_sample_rate(0.0),
	  d_realtime_sample_count(0),
	...
	// Do the open action
	open_immediate(filespecs, alphabetical, repeat,
	               terminate_at_end);
}
...
int vita_multifile_iq_source_impl::read_output_items_immediate(gr_complex *out) {
	int noutput_items_processed = 0;
	int bytes_read = 0;
	...
	if ( d_buffer_offset >= d_packet_size ) {
		...
		volk_16i_s32f_convert_32f((float*)out, vp.sampleData, 1/d_iq_scale_factor, samples_in_packet * 2);
#endif
...
void vita_multifile_iq_source_impl::generate_vita_tags(int output,
	...
	add_item_tag(
	    ...
	    pmt::from_uint64(d_absolute_packet_num),
	    srcid
	);
	add_item_tag(
	    output,
	    absolute_sample_num,
	    pmt::string_to_symbol("filename"),
	    pmt::string_to_symbol(d_filenames[d_filename_index]),
	    srcid
	);
	add_item_tag(
	    output,
