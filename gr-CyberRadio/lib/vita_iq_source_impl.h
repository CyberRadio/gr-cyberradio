...
public:
  vita_iq_udp_port(const std::string &host = "0.0.0.0", int port = 40001,
                   int packet_size = 8192, bool debug = false);
  ~vita_iq_udp_port();
  void read_data();
  void clear_buffer();
...
public:
  vita_iq_source_impl(int vita_type, size_t payload_size,
                      size_t vita_header_size, size_t vita_tail_size,
                      bool byte_swapped, bool iq_swapped, float iq_scale_factor,
                      const std::string &host,
                      const std::vector<unsigned short> &port_list, bool tagged,
                      bool debug);
  ~vita_iq_source_impl();
