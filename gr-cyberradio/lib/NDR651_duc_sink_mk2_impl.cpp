	...
	namespace CyberRadio
	{
			...
			d_sink(NULL)
		{
			d_sink = new LibCyberRadio::NDR651::DUCSink(
							  "[CyberRadio] NDR651 DUC Sink (Mk2)",
							  radio_host_name,
							  radio_tcp_port,
							  tengig_iface_list,
							  iq_scale_factor,
		...
		std::vector<std::string> NDR651_duc_sink_mk2_impl::get_tengig_iface_list() const
		{
		...
		void NDR651_duc_sink_mk2_impl::set_duc_tx_channels(unsigned int duc_tx_channels)
		{
			d_sink->set_duc_tx_channels(duc_tx_channels);
		}

		unsigned int NDR651_duc_sink_mk2_impl::get_duc_tx_frequency() const
		{
			return d_sink->get_duc_tx_frequency();
		}

		void NDR651_duc_sink_mk2_impl::set_duc_tx_frequency(unsigned int duc_tx_frequency)
		{
		...
		bool NDR651_duc_sink_mk2_impl::start()
		{
			return d_sink->start();
		}

		bool NDR651_duc_sink_mk2_impl::stop()
		{
			return d_sink->stop();
		}
