	...
	namespace CyberRadio
	{
		namespace NDR651
		{
				...
				_constructing(true)
			{
				...
				_fcClient = new FlowControlClient(ducChannel, _config_tx, 20, _debug);
			...
			{
				this->debug("starting flow controller\n");
				if ( _fcClient != NULL )
					_fcClient->start();
			}
			...
			{
				this->debug("stopping flow controller\n");
				if ( _fcClient != NULL )
					_fcClient->interrupt();
			}
			...
			unsigned int TransmitPacketizer::sendFrame(short * samples)
			{
				...
				if ( (_txSock != NULL) && (_fcClient != NULL) )
				{
					if (!_fcClient->okToSend(128*SAMPLES_PER_FRAME,false))
					{
						//_waiting = true;
					}
					else if (_fcClient->okToSend(SAMPLES_PER_FRAME,true))
					{
						//if (_waiting)
						//	_waiting = false;
						...
						_fcClient->sentNSamples(_samplesSent);
					}
				}
				return _samplesSent;
			}
			...
			int TransmitPacketizer::debug(const char *format, ...)
			{
				int ret = 0;
				if (_debug)
				{
					ret = fprintf(stderr, "[%lu][TransmitPacketizer] ",
							      time(NULL));
					if (ret >= 0)
					{
						va_list ap;
