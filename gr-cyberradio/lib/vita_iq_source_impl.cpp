...
namespace gr
{
	...
	{
		vita_iq_udp_port::vita_iq_udp_port(const std::string& host,
						                   int port,
						                   int packet_size,
										   bool debug) :
			host(host),
		...
		vita_iq_source::sptr vita_iq_source::make(int vita_type,
                                              size_t payload_size,
											  ...
											  bool debug)
		{
			return gnuradio::get_initial_sptr(new vita_iq_source_impl(
					...
					iq_scale_factor, host, port_list, tagged, debug) );
		}

		/*
		 * The private constructor
		 */
		vita_iq_source_impl::vita_iq_source_impl(int vita_type,
											 size_t payload_size,
											 size_t vita_header_size,
											 size_t vita_tail_size,
											 bool byte_swapped,
		          ...
		          d_debug(debug)
		{
			...
			//this->set_output_signature(gr::io_signature::make(1, d_num_outputs, sizeof(unsigned short) * d_payload_size / 2));
				...
				do
				{
					...
					for (output = 0; output < d_num_outputs; output++)
					{
						...
						if ( d_udp_ports[output]->is_packet_ready() )
						{
							...
							{
								out[sample + noutput_items_processed[output]].real(vp.getSampleI(sample) * d_iq_scale_factor);
								out[sample + noutput_items_processed[output]].imag(vp.getSampleQ(sample) * d_iq_scale_factor);
							}
					...
					max_noutput_items_processed = *std::max_element(
							noutput_items_processed.begin(),
							noutput_items_processed.end() );
				...
				if ( max_noutput_items_processed > 0)
				{
					for (output = 0; output < d_num_outputs; output++)
					{
						...
						for (sample = noutput_items_processed[output];
							 sample < max_noutput_items_processed;
							 sample++)
						{
							out[sample].real(0.0);
							out[sample].imag(0.0);
		...
		void vita_iq_source_impl::connect_udp_ports()
		{
			...
			for (int i = 0; i < d_num_outputs; i++)
			{
				...
				d_udp_ports.push_back( new vita_iq_udp_port(d_host,
						d_port_list[i], d_packet_size, d_debug) );
